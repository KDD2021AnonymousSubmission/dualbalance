...
def plot_scatter(X, y, ax=None, weights=None, title='',
    ...
    title += ' (2D projection by {})'.format(str(projection.__class__).split('.')[-1][:-2])
    size = 50 if weights is None else weights
    sns.scatterplot(x=X_vis[:, 0], y=X_vis[:, 1], hue=y, style=y, s=size, **vis_params, legend='full', ax=ax)
...
def plot_class_distribution(y, ax=None, title='', sort_values=False, plot_average=True):
